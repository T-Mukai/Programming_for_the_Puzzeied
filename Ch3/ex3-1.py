#Exercise1
#The problem with this particular strategy is that it does not check that the five cards obtained are distinct.
#In fact, if you input the number 888888, the following five cards are generated in this order: ['A_C', 'A_C', '7_H', 'J_D', 'K_S'].
#You can see the problem here – we are not checking for duplicate cards in ComputerAssistant.
#Fix this procedure to check for distinct cards, and generate additional “random” numbers by running the loop above for additional iterations till five distinct cards are generated.
