...
#Exercise 1 change output. Output is the maximam number in the party when you are in it [ystart ,yend) .
...
#ex2-1 ystart and yend are the times when you are in the party.
def bestTimeToPartySmart(schedule , ystart, yend):
    ...
    times.append((ystart, 'ystart'))
    times.append((yend, 'yend'))
...
def chooseTime(times, ystart, yend):
    ...
    maxcount = 0
    time = 0
    #maxcount is considered only in (ystart, yend]. watch = 0 means not considering.
    watch = 0
    ...
    for t in times:
        ...
        if watch == 1 and rcount > maxcount:
            maxcount = rcount
            time = t[0]

    return maxcount, time

##bestTimeToPartySmart(sched, 7.0 , 9.0)
