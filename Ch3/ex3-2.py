#Exercise 2:
#Modify ComputerAssistant so in the case of two pairs of cards with the same suit,
#the hidden and first cards are selected such that the number to be encoded is the smallest possible.
...
##    print (cards)
    ...
    ##ex2
    mindiff = 53
    pairsuit = set(pairsuit)
    pairsuit = list(pairsuit)
    if len(pairsuit) > 1:
        for i in pairsuit:
            sub = []
            for j in range(5):
                if cardsuits[j] == i:
                    sub.append(j)
            ## for debugging
            # print(deck[cind[sub[0]]],deck[cind[sub[1]]])
            # print(cnumbers[sub[0]],cnumbers[sub[1]])
            diff = (cnumbers[sub[0]] - cnumbers[sub[1]])
            if diff < 0:
                diff = -diff
            if diff < mindiff:
                mindiff = diff
